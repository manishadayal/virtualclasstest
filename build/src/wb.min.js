...
()=>{virtualclass.popup.closeElem()})};a.prototype.replayWindowAction=function(a){document.getElementById("replayContainer").style.display=a};a.prototype.progressBarAction=function(a){document.getElementById("recordingContainer").style.display=a};a.prototype.confirmInput=function(a,b,d,c,f,t){var g=document.getElementById("about-modal");virtualclass.popup.open(g);this.hideAllPopups();g=document.getElementById("confirm");g.style.display="block";var k=g.getElementsByClassName("confirmChild");if(0<k.length)for(;1<=
...
document.querySelector(".vceditor-toolbar-wrapper .open");null!=a&&(a.classList.remove("open"),a.classList.add("close"))}}}})(window);(function(c,b){c.Chat=function(){let a;return{userList:[],init(){this.chatroombox=null;this.chatWindow="private";this.counter=0;this.idList=[];this.commonChat=[];this.token=wbUser.token;b.htab=[];b.htabIndex=[];this.vmstorage={};b("body").footerbar();a=b("#tabs").tabs({cache:!0,activeOnAdd:!0});!1===browserSupportsLocalStorage()?alert(virtualclass.lang.getString("sterror")):(this.history(),this.UI.privateChatBox.init(),b("ul.tabs").on("click, focus","li",function(){b(`li[aria-controls='${b(this).attr("id")}']`).removeClass("ui-state-highlight")}),
roles.isStudent()&&(virtualclass.settings.studentpc(virtualclass.settings.info.studentpc),virtualclass.settings.studentgc(virtualclass.settings.info.studentgc)))},fetchChatUsers(){const a=new FormData;a.append("action","fetchUser");a.append("token",this.token);virtualclass.xhr.vxhr.post(`${c.webapi}&user=${virtualclass.gObj.uid}&methodname=core_course_get_enrolled_users_by_cmid`,a).then(a=>{virtualclass.chat.userList=a.data;updateOnlineUserText()}).catch(a=>{console.error("Request failed with error ",
a)})},setChatDisplay(){const a=document.querySelector("#virtualclassCont.congrea  #ta_chrm2"),b=document.querySelector("#virtualclassCont.congrea #congreaUserSearch");var c=document.getElementById("memlist");document.getElementById("user_list");var k=localStorage.getItem("chatWindow");null!=k&&"common"===k?(virtualclass.chat.chatWindow="common",(k=document.getElementById("chatrm"))?(k.classList.add("enable"),k.classList.remove("disable"),document.getElementById("chatroom_bt2").classList.add("active"),
c&&(c.classList.remove("enable"),c.classList.contains("disable")||c.classList.add("disable"),c=document.getElementById("user_list"),c.classList.remove("active")),b&&(b.style.display="none")):c&&(c.classList.add("enable"),c.classList.remove("disable"),c=document.getElementById("user_list"),c.classList.add("active"))):null!=k&&"support"===k?(c=document.getElementById("congreaSupport"))&&c.classList.add("active"):(c&&(c.classList.add("enable"),document.getElementById("user_list"),virtualclass.chat.chatWindow=
"private"),a&&(a.style.display="none"));document.querySelector("#virtualclassCont.congrea  .vmchat_bar_button").classList.contains("active")?(a&&(a.style.display="none"),b&&(b.style.display="block")):(a&&(a.style.display="block"),b&&(b.style.display="none"))},showChatListUsers(){const a=virtualclass.chat.userList;0<a.length&&(this.showChatUserList(a),displayChatOfflineUserList(a),document.querySelector("div#memlist").classList.add("enable"))},showChatUserList(a){if(!document.getElementById("chat_div")){const a=
document.createElement("div");a.id="chat_div";document.body.appendChild(a)}b("#chat_div").memberlist({id:"chat_div",user:a,offset:"-1px",title:virtualclass.lang.getString("online"),userSent(a){return b("#chat_div").memberlist("option","boxManager").addUsr(a)}})},history(){var a=null;null!=localStorage.getItem(wbUser.sid)&&(a=JSON.parse(localStorage.getItem(wbUser.sid)),displayPvtChatHistory(a),a=localStorage.getItem("chatEnable"),null!=a&&"false"===a&&virtualclass.user.control.disbaleAllChatBox(),
this.vmstorage=JSON.parse(localStorage.getItem(wbUser.sid)));0<localStorage.length&&(displaycomChatHistory(),virtualclass.chat.removeChatHighLight("chatrm"),null!=a&&"false"===a&&virtualclass.user.control.disableCommonChat())},exportCommonChat(a){const b=[],d=virtualclass.chat.commonChat;for(let c=0;c<d.length;c++)a<=d[c].time&&b.push(d[c]);return b},UI:{privateChatBox:{init(){b("#tabs").delegate("span.icon-close","click",this.close);b("#tabs").on("click","li a",this.toggle)},close(){var a=b(this).closest("li").attr("id").substring(5);
...
function displayChatOfflineUserList(c){alert("display chat offline user");let b=document.querySelector("#melistcontainer");null==b&&(b=document.createDocumentFragment("div"),b.id="melistcontainer");for(let a=0;a<c.length;a++)virtualclass.vutil.alreadyConnected(c[a].id)||c[a].id==virtualclass.gObj.uid||null!=document.getElementById(`video${c[a].id}`)||(myDivResult=$("#chat_div").memberlist("option").userSent(c[a]),virtualclass.media.addUserRole(myDivResult,c[a].role),b.appendChild(myDivResult));document.querySelector("#chat_div").appendChild(b)}
function updateOnlineUserText(){roles.hasAdmin()&&0<virtualclass.chat.userList.length&&(document.querySelector("#userListHeader #onlineusertext").innerHTML="");document.querySelector("#userListHeader #onlineusertext").innerHTML=`(${virtualclass.connectedUsers.length})`}
...
b.classList.remove("vidShow")}}}};(function(c){"use strtic";var b=function(){return{coursePoll:[],sitePoll:[],setting:{showResult:!0,timer:!0,time:{}},uid:0,currQid:0,currOption:{},dataToStd:{},count:{},save:0,newUserTime:{},list:[],listPoll:{},currResultView:"bar",tStamp:[],pollState:{},uniqueUsers:[],init(){this.pollState={};virtualclass.previrtualclass="virtualclassPoll";virtualclass.previous=virtualclass.previrtualclass;this.cmid=virtualclass.vutil.getUrlVars(c.webapi).cmid;this.token=wbUser.token;this.timer&&clearInterval(this.timer);
roles.isStudent()?this.UI.defaultLayoutForStudent():(this.UI.container(),ioAdapter.mustSend({poll:{pollMsg:"init"},cf:"poll"}));!virtualclass.isPlayMode&&roles.hasControls()&&this.interfaceToFetchList(this.cmid)},interfaceToSave(a){const b=this,f=new FormData;f.append("dataToSave",JSON.stringify(a));f.append("user",virtualclass.gObj.uid);f.append("token",b.token);virtualclass.xhr.vxhr.post(`${c.webapi}&methodname=mod_congrea_poll_save`,f).then(a=>{a=a.data;const {options:d}=a,c={},f={};d.forEach(a=>
{f[a.optid]=a.options});a.copied?(a.options=f,c.questionid=a.qid,c.category=a.category,c.createdby=a.createdby,c.questiontext=a.question,c.creatorname=a.creatorname,c.options=a.options,b.publishPoll(c),b.interfaceToFetchList(c.category)):b.updatePollList(a);b.currQid=a.qid;b.currOption=f}).catch(a=>{console.error("Request failed with error ",a)})},interfaceToEdit(a){const b=this,f=new FormData;f.append("editdata",JSON.stringify(a));f.append("user",virtualclass.gObj.uid);f.append("token",b.token);
virtualclass.xhr.vxhr.post(`${c.webapi}&methodname=mod_congrea_poll_update`,f).then(a=>{b.updatePollList(a.data)}).catch(a=>{console.error("Request failed with error ",a)})},updatePollList(a){const {options:b}=a,c={},g={};b.forEach(a=>{g[a.id]=a.options});c.questionid=a.qid;c.category=a.category;c.createdby=a.createdby;c.questiontext=a.question;c.creatorname=a.creatorname;c.options=g;this.currQid=a.qid;this.currOption=g;(c.category?this.coursePoll:this.sitePoll).push(c);this.interfaceToFetchList(c.category)},
interfaceToFetchList(a){const b=this,f=new FormData;f.append("category",parseInt(a));f.append("user",virtualclass.gObj.uid);f.append("token",b.token);virtualclass.recorder.items=[];virtualclass.xhr.vxhr.post(`${c.webapi}&methodname=mod_congrea_poll_data_retrieve`,f).then(d=>{d=d.data;for(let a=0;a<=d.length-1;a++){const {options:b}=d[a];for(var c in b)d[a].options[c]=b[c].options}c=d.pop();JSON.parse(a)?(b.coursePoll=d,b.displaycoursePollList()):(b.sitePoll=d,b.displaysitePollList(c))}).catch(a=>
{console.error("Request failed with error ",a)})},interfaceToDelete(a){const b=this,f=new FormData;f.append("qid",JSON.stringify(a));f.append("user",virtualclass.gObj.uid);f.append("token",b.token);virtualclass.xhr.vxhr.post(`${c.webapi}&methodname=mod_congrea_poll_delete`,f).then(a=>{const d=a.data;a.data&&""!==a.data&&b.interfaceToFetchList(d)}).catch(a=>{console.error("Request failed with error ",a)})},interfaceToDelOption(a){const b=new FormData;b.append("id",JSON.stringify(a));b.append("user",
virtualclass.gObj.uid);b.append("token",this.token);virtualclass.xhr.vxhr.post(`${c.webapi}&methodname=mod_congrea_poll_option_drop`,b).then(()=>{}).catch(a=>{console.error("Request failed with error ",a)})},interfaceToSaveResult(a){const b=this,f=new FormData;f.append("saveResult",JSON.stringify(a));f.append("user",virtualclass.gObj.uid);f.append("token",b.token);virtualclass.xhr.vxhr.post(`${c.webapi}&methodname=mod_congrea_poll_result`,f).then(a=>{(""!==a.data||"OK"===a.statusText&&0===a.data)&&
b.interfaceToFetchList(a.data)}).catch(a=>{console.error("Request failed with error ",a)})},reloadPollList(a,b){a=document.getElementById("coursePollTab");const d=document.getElementById("sitePollTab");let c=0;"course"===b?(d.classList.remove("active"),a.classList.add("active"),c=this.cmid):(a.classList.remove("active"),d.classList.add("active"));this.interfaceToFetchList(c)},reloadVotedScrn(){this.dataRec.newTime=0;var a=document.getElementById("stdPollMszLayout");a&&a.parentNode.removeChild(a);
if(a=document.getElementById("stdPollContainer"))a.style.display="none";a=this.dataRec.setting.showResult?virtualclass.lang.getString("votesuccess"):virtualclass.lang.getString("votesuccessPbt");this.showMsg("mszBoxPoll",a,"alert-success");this.pollState.currScreen="voted";this.pollState.data=this.dataRec;this.pollState.timer=this.newUserTime},reloadStdResult(a){const b=document.getElementById("mszBoxPoll");b&&(b.style.display="none");if(this.dataRec=a.data)this.dataRec.newTime=a.data.timer;this.count=
a.data.count;console.log("====> student submit poll ",this.count);this.currResultView=a.data.view;this.stdPublishResult(this.count);"bar"===this.currResultView?this.updateBarGraph():"pi"===this.currResultView&&(this.createPiChart(),this.updatePiChart());this.pollState.currScreen="stdPublishResult";this.pollState.data=this.dataRec;this.pollState.timer=this.newUserTime;this.pollState.count=this.count;this.pollState.view=this.currResultView},loadTeacherScrn(a){a.data.view=a.data.view||"bar";this.dataToStd.question=
a.data.question;this.dataToStd.options=a.data.options;this.dataToStd.qId=a.data.qId;this.setting=a.data.setting;this.nTimer=this.newTime=this.newUserTime=a.data.newTime;this.currResultView=a.data.view;this.uniqueUsers=a.data.users;var {pollType:b}=a.data;const c=document.getElementById("coursePollTab"),g=document.getElementById("sitePollTab");let k=0;"course"===b?(g.classList.remove("active"),c.classList.add("active"),k=this.cmid):(c.classList.remove("active"),g.classList.add("active"));this.interfaceToFetchList(k);
this.list=a.data.list;b={question:this.dataToStd.question,options:this.dataToStd.options,setting:this.setting,newTime:this.nTimer,count:this.count,view:this.currResultView,list:this.list,totalUsers:a.data.totalUsers,users:this.uniqueUsers,pollType:b,qId:a.data.qId};console.log("====> student submit poll ",this.count);"yes"!==a.pollClosed&&this.reloadTeacherPublish(a);"undefined"!==typeof a.data.pollClosed&&"yes"!==a.pollClosed&&(this.UI.pollClosedUI(),a=virtualclass.lang.getString("Pclosed"),this.showMsg("resultLayoutHead",
a,"alert-success"),this.timer&&clearInterval(this.timer),this.testNoneVoted(virtualclass.poll.count),(a=document.getElementById("pollResultMsz"))&&a.parentNode.removeChild(a),b.pollClosed="yes",this.pollState.pollClosed="yes");this.pollState.data=b;this.pollState.currScreen="teacherPublish";localStorage.removeItem("pollState")},reloadTeacherPublish(a){const {pollType:b}=a.data;virtualclass.modal.closeModalHandler("editPollModal");const c=this.setting.timer;this.UI.resultView(c,b);this.list=a.data.list;
this.count=a.data.count;this.currResultView=a.data.view;this.reloadGraph();this.updateVotingInformation();if(c){const [b,c,d]=this.getFormatedTime();if(c||1<d){if(a=document.getElementById("timerWrapper")){var g=document.createElement("div");g.id="timerCont";a.appendChild(g)}this.remainingTimer({hour:b,min:c,sec:d})}else if(c||1>=d){if(a=document.getElementById("timerWrapper"))g=document.createElement("div"),g.id="timerCont",a.appendChild(g);g.innerHTML=`${10>c?`0${c}`:c}:${10>d?"00":d}`;(g=document.getElementById("pollResultMsz"))&&
g.parentNode.removeChild(g)}}else this.triggerElapseTimer();if(g=document.getElementById("modalClose"))g.removeAttribute("data-dismiss"),g.addEventListener("click",()=>{virtualclass.poll.pollModalClose(b)});this.pollState.currScreen="teacherPublish"},pollModalClose(a){roles.hasControls()&&virtualclass.poll.pollState.currScreen&&"teacherPublish"===virtualclass.poll.pollState.currScreen&&(virtualclass.poll.pollState.currScreen="course"===a?"displaycoursePollList":"displaysitePollList");const b=virtualclass.lang.getString("pclosetag");
virtualclass.popup.confirmInput(b,this.resultCloseConfirm,a)},resultCloseConfirm(a){if(a){(a=document.querySelector("#editPollModal"))&&a.remove();virtualclass.poll.timer&&(ioAdapter.mustSend({poll:{pollMsg:"stdPublishResult",data:virtualclass.poll.count},cf:"poll"}),virtualclass.poll.resultToStorage(),virtualclass.poll.interfaceToSaveResult({qid:virtualclass.poll.dataToStd.qId,list:virtualclass.poll.list}),clearInterval(virtualclass.poll.timer),virtualclass.poll.timer=0);if(a=document.getElementById("congreaPollVoters"))a.innerHTML=
virtualclass.lang.getString("receivedVotes");virtualclass.poll.count={};ioAdapter.mustSend({poll:{pollMsg:"completeClose"},cf:"poll"})}},resultToStorage(){(roles.hasControls()?this.dataToStd:this.dataRec)&&this.uid++},reloadGraph(){"bar"===this.currResultView?(this.showGraph(),this.updateBarGraph()):"pi"===this.currResultView?(this.createPiChart(),this.updatePiChart()):"list"===this.currResultView&&this.listView();var a=document.getElementsByClassName("emptyList");if(0<this.list.length)for(var b=
...
virtualclass.poll[a.poll.pollMsg].call(this,a.poll.data,b):virtualclass.poll[a.poll.pollMsg].call(this,a.poll.data,b)},pollPopUp(a,b,c){const d=function(){virtualclass.poll.action(this.id,a,b,c);"goBack"!==this.id&&"cacelSetting"!==this.id||virtualclass.modal.removeModal()};var f=document.getElementById("editPollModal");f=(document.getElementById("editPollModal")?f:document.getElementById("qnPopup")).querySelectorAll("#pollModalBody .controls");for(let a=0;a<f.length;a++)f[a].addEventListener("click",
d)},attachConfirmInit(a,b,c,g){this.action(a,b,c,g)},action(a,b,c,g){b(a,c,g)},displaycoursePollList(){var a;const b=this;this.dispList("course");if(a=document.querySelector("#listQnContcourse tbody"))for(;a.childNodes.length;)a.removeChild(a.lastChild);const c=document.querySelector("#listQnContcourse");if(this.coursePoll.length)c&&(c.style.display="block"),a=document.querySelector("#mszBoxPoll"),a.style.display="none",this.coursePoll.forEach((a,c)=>{b.forEachPoll(a,c,"course")});else{a=document.querySelector("#mszBoxPoll");
const b=virtualclass.lang.getString("noPoll");a.style.display="block";a.innerHTML=b;c&&(c.style.display="none")}(a=document.getElementById("emptyListsite"))&&a.parentNode.removeChild(a);this.dispNewPollBtn("course");"teacherPublish"!==this.pollState.currScreen&&(this.pollState.currScreen="displaycoursePollList")},displaysitePollList(a){var b;const c=this;this.dispList("site");if(b=document.querySelector("#listQnContsite tbody"))for(;b.childNodes.length;)b.removeChild(b.lastChild);const g=document.querySelector("#listQnContsite");
if(this.sitePoll.length)this.sitePoll.forEach((b,d)=>{c.forEachPoll(b,d,"site",a)}),g&&(g.style.display="block"),b=document.querySelector("#mszBoxPoll"),b.style.display="none";else{b=document.querySelector("#mszBoxPoll");let c=virtualclass.lang.getString("noPoll");"false"===a&&(c=virtualclass.lang.getString("noPollNoAdmin"));b.style.display="block";b.innerHTML=c;g&&(g.style.display="none")}(b=document.getElementById("emptyListcourse"))&&b.parentNode.removeChild(b);this.dispNewPollBtn("site",a);"teacherPublish"!==
this.pollState.currScreen&&(this.pollState.currScreen="displaysitePollList")},dispList(a){var b=document.getElementById("mszBoxPoll");0<b.childNodes.length&&b.childNodes[0].parentNode.removeChild(b.childNodes[0]);b="course"===a?"site":"course";this.hidePollList(b);var c=document.getElementById(`listQnCont${a}`);c&&(c.style.display="table",(c=document.getElementById(`newPollBtn${a}`))&&c.classList.contains(b)&&(c.classList.remove(b),c.classList.add(a)))},dispNewPollBtn(a,b){this.attachEvent(`newPollBtn${a}`,
"click",this.newPollHandler,a);var c=document.getElementById(`newPollBtn${a}`);"site"===a?("undefined"!==typeof b&&"true"===b?c.style.display="table":(c.style.display="none",roles.hasControls&&(a=document.getElementById("listQnContsite"),a.classList.add("teacherTableHeader"))),c=document.getElementById("newPollBtncourse")):(roles.hasControls&&(a=document.getElementById("listQnContsite"),a.classList.remove("teacherTableHeader")),c.style.display="table",c=document.getElementById("newPollBtnsite"));
c&&(c.style.display="none")},forEachPoll(a,b,c,g){const d={};d.questiontext=a.questiontext;var f=Object.prototype.hasOwnProperty.call(a,"creatorfname")?virtualclass.poll.capitalizeFirstLetterFnameLname(a.creatorfname):virtualclass.poll.capitalizeFirstLetterFnameLname(a.creatorname);d.creator=f;d.pollType=c;d.index=b;f=virtualclass.getTemplate("qn","poll");(document.querySelector(`#listQnCont${c} .pollList tbody`)||document.querySelector(`#listQnCont${c} .pollList`)).insertAdjacentHTML("beforeend",
f({pollQn:d}));if("course"===c&&a.createdby===wbUser.id||"site"===c&&"true"===g){if(a.isPublished){if(g=document.querySelector(`#editQn${c}${b} span`))g.setAttribute("title",virtualclass.lang.getString("etDisabledA")),g.style.cursor="default",g.classList.add("disabled")}else this.attachEvent(`editQn${c}${b}`,"click",this.editHandler,a,c,b,a.createdby,a.questionid);this.attachEvent(`deleteQn${c}${b}`,"click",this.deleteHandler,a,c,b)}else{if(g=document.querySelector(`#editQn${c}${b} span`))g.setAttribute("title",
virtualclass.lang.getString("etDisabledCr")),g.style.cursor="default",g.classList.add("disabled");g=document.querySelector(`#deleteQn${c}${b} span`);g.setAttribute("title",virtualclass.lang.getString("dltDisabled"));g.style.cursor="default";g.classList.add("disabled")}g="course"==c?virtualclass.poll.coursePoll:virtualclass.poll.sitePoll;b!==g.length-1&&b!==g.length-2&&b!==g.length-3||document.querySelector(`#contQn${c}${b}`).classList.add("lastNode");this.attachEvent(`publishQn${c}${b}`,"click",this.publishHandler,
a,c,b);this.previewOnHover(a,c,b)},previewOnHover(a,b,c){const d={};document.querySelector(`#qnText${b}${c} .popover-content`)||(d.questiontext=a.questiontext,d.options=a.options,a=virtualclass.getTemplate("previewPopup","poll")({data:d}),virtualclass.modal.attachpopupHandler(b,c,a))},attachEvent(a,b,c,g,k,m,n,r,t){r=document.getElementById(a);null!=r&&r.addEventListener(b,()=>{"undefined"!==typeof g?c(g,k,m,a,n,t):c(k,m,a,n,t)})},hidePollList(a){if(a=document.getElementById(`listQnCont${a}`))a.style.display=
...
a)},popupFn(a,b,c){"goBack"!==a&&virtualclass.poll[a].call(virtualclass.poll,b,c,a)},next(a,b){virtualclass.poll.pollSetting(b,a)},etSave(a,b,c){if(!virtualclass.poll.isBlank())return 0;const d="course"===b?virtualclass.poll.coursePoll[a]:virtualclass.poll.sitePoll[a],f="course"===b?virtualclass.poll.cmid:0,m=document.getElementById("q");document.getElementById("etSave").setAttribute("data-dismiss","modal");if("undefined"!==typeof a)virtualclass.poll.saveQuestion(m,a,b),virtualclass.poll.saveOption(a,
b),virtualclass.poll.interfaceToEdit({questionid:d.questionid,question:d.questiontext,options:d.options,createdby:d.createdby},f);else if(!virtualclass.poll.newPollSave("undefined",b))return 0;"etSave"===c&&virtualclass.modal.removeModal();return 1},closePoll(a){const b=virtualclass.lang.getString("pclose");virtualclass.popup.confirmInput(b,virtualclass.poll.askConfirmClose,"close",a)},saveQuestion(a,b,c){"course"===c?(this.coursePoll[b].questiontext=a.value,this.coursePoll[b].creator=wbUser.name):
(this.sitePoll[b].questiontext=a.value,this.sitePoll[b].creator=wbUser.name);document.getElementById(`qnText${c}${b}`).innerHTML=a.value},saveOption(a,b){const c=[];let d=0;var k;b="course"===b?this.coursePoll[a]:this.sitePoll[a];const m=document.getElementById("optsTxCont").querySelectorAll("#virtualclassCont #optsTxCont .opt");for(a=0;a<m.length;a++)c[a]=m[a].value;for(k in b.options)b.options[k]=c[d],d++;if(a>d)for(d,k=d;k<a;k++)b.options[`noid${d}`]=c[d],d++},hideModal(){virtualclass.modal.removeModal()},
...
let b=0,c=0;for(let d=0;d<a.length;d++)null==a[d].value||""===a[d].value.trim()?c++:b++;a=document.getElementById("q");return null==a.value||""===a.value.trim()?(virtualclass.lang.getString("pollblank"),alert(virtualclass.lang.getString("pollblank")),0):2>b?(alert(virtualclass.lang.getString("minoption")),0):c?(alert(virtualclass.lang.getString("delblank")),0):1},saveNdPublish(a,b){if(!virtualclass.poll.isBlank())return 0;var c=`${b}Poll`,d=document.getElementById("optsTxCont").querySelectorAll("#virtualclassCont #optsTxCont .opt");
if("undefined"===typeof a){({length:c}=virtualclass.poll[c]);var k=document.getElementById("q").value,m={};for(var n=0;n<d.length;n++)m[n]=d[n].value;d=m;virtualclass.poll.dataToStd.question=k;virtualclass.poll.dataToStd.options=d;virtualclass.poll.newPollSave(c,b,!0)&&virtualclass.poll.pollSetting(b,c,!0)}else{k=virtualclass.poll[c][a];n=document.getElementById("q").value;k.question=n;n=0;for(m in k.options)k.options[m]=d[n].value,n++;virtualclass.poll[c][a].questiontext=document.getElementById("q").value;
virtualclass.poll.dataToStd.question=virtualclass.poll[c][a].questiontext;virtualclass.poll.dataToStd.options=virtualclass.poll[c][a].options;virtualclass.poll.etSave(a,b,setting)&&virtualclass.poll.pollSetting(b,a,!0)}a={type:"course",qid:a,pollqnOps:virtualclass.poll.dataToStd};virtualclass.poll.pollState.currScreen="setting";virtualclass.poll.pollState.data=a},pollCancel(){virtualclass.popup.closeElem();virtualclass.modal.closeModalHandler("editPollModal")},reset(){const a=document.querySelectorAll("#editPollModal textArea");
for(let b=0;b<a.length;b++)a[b].value=""},addMoreOption(a,b){virtualclass.poll.UI.createOption(a,b)},removeOption(a,b,c){const d=c.replace("remove","");c=document.getElementById(c);c.parentNode.parentNode.removeChild(c.parentNode);a="course"===a?virtualclass.poll.coursePoll[b]:virtualclass.poll.sitePoll[b];"undefined"!==typeof a&&(delete a.options[d],virtualclass.poll.interfaceToDelOption(d))},publishHandler(a,b,c){var d=document.getElementById("mszBoxPoll");0<d.childNodes.length&&d.childNodes[0].parentNode.removeChild(d.childNodes[0]);
({isPublished:d}=a);d?virtualclass.poll.duplicatePoll(a):virtualclass.poll.publishPoll(a,b,c)},duplicatePoll(a){const b=[];for(const c in a.options)b.push(a.options[c]);virtualclass.poll.interfaceToSave({question:a.questiontext,options:b,action:"newPollSave",category:a.category,copied:!0},a.category)},publishPoll(a,b){virtualclass.poll.dataToStd.question=a.questiontext;virtualclass.poll.dataToStd.qId=a.questionid;virtualclass.poll.dataToStd.options=a.options;document.getElementById("layoutPollBody");
var c=document.getElementById("editPollModal");c&&c.remove();c={};c.questiontext=a.questiontext;c.options=a.options;const d=virtualclass.getTemplate("preview-modal","poll"),k=document.querySelector("#virtualclassApp #bootstrapCont");k&&k.insertAdjacentHTML("beforeend",d({poll:c}));b||(b="0"===a.category?"site":"course");virtualclass.poll.pollPreview(b);virtualclass.modal.closeModalHandler("editPollModal")},pollPreview(a){const b=document.getElementById("contFooter");virtualclass.poll.UI.previewFooterBtns(b,
a)},deleteHandler(a,b,c){a=document.getElementById("mszBoxPoll").querySelectorAll("#mszBoxPoll .alert");0<a.length&&a[0].parentNode.removeChild(a[0]);a=virtualclass.lang.getString("pollDel");virtualclass.popup.confirmInput(a,virtualclass.poll.askConfirm,b,c)},showMsg(a,b,c){let d=document.getElementById(a);if(!d){const b=document.getElementById("layoutPollBody");d=document.createElement("div");d.id=a;b.appendChild(d)}a=document.createElement("div");a.className="alert  alert-dismissable";a.classList.add(c);
a.innerHTML=b;d.insertBefore(a,d.firstChild)},askConfirm(a,b,c){if(a){a=document.getElementById(`contQn${b}${c}`);a.parentNode.removeChild(a);a=virtualclass.lang.getString("Pdsuccess");virtualclass.poll.showMsg("mszBoxPoll",a,"alert-success");a="course"===b?virtualclass.poll.coursePoll[c]:virtualclass.poll.sitePoll[c];const d=a.questionid;a="course"===b?virtualclass.poll.coursePoll:virtualclass.poll.sitePoll;a.splice(c,1);virtualclass.poll.interfaceToDelete(d)}},askConfirmClose(a,b,c){if(a){(virtualclass.poll.setting.showResult&&
...
a=virtualclass.poll.timer=0;for(var d in virtualclass.poll.count)virtualclass.poll.count[d]&&(a=1);if(a)"list"!==virtualclass.poll.currResultView&&virtualclass.poll.showChart();else{virtualclass.poll.noneVoted(c);if(c=document.getElementById("resultLayoutHead"))for(d=0;d<c.childNodes.length;d++)c.childNodes[d].parentNode.removeChild(c.childNodes[d]);c=document.getElementById("pollResultMsz");c.parentNode.removeChild(c);c=document.getElementById("chartMenuCont");c.parentNode.removeChild(c)}}},showStudentPollReport(a){virtualclass.poll.studentReportLayout(a);
(a=document.getElementById("mszBoxPoll"))&&a.parentNode.removeChild(a)},studentReportLayout(a){for(var b=document.getElementById("layoutPollBody");1<b.childElementCount;)b.removeChild(b.lastChild);const c=document.createElement("div");b.appendChild(c);b=a.result;virtualclass.poll.count=b;console.log("====> student submit poll ",virtualclass.poll.count);virtualclass.poll.dataRec=a.pollData;virtualclass.poll.stdPublishResult(b,"true")},timerDisable(){document.getElementById("timer").classList.add("disabled")},
...
(d++,60===d&&(d=0,c++,60===c&&(c=0)),k.innerHTML=`${10>c?`0${c}`:c}:${10>d?`0${d}`:d}`):clearInterval(virtualclass.poll.timer)};a();virtualclass.poll.timer=setInterval(a,1E3)},remainingTimer(a){virtualclass.poll.timer&&clearInterval(virtualclass.poll.timer);const b=document.getElementById("timerCont");let c=a.min||0,g=a.sec||0;a=function(){b?(b.innerHTML=`${10>c?`0${c}`:c}:${10>g?`0${g}`:g}`,g--,0>=g&&(g=59,c--,0>c&&(g=c=0,b.innerHTML=`${10>c?`0${c}`:c}:${10>g?`0${g}`:g}`,clearInterval(virtualclass.poll.timer),
...
virtualclass.poll.uniqueUsers.length:0)&&(a=c);c=document.getElementById("receivedVotes");null!=c&&(c.innerHTML=`${b}\/${a}`)},updateBarGraph(){const a=document.getElementById("pollResultMsz"),b=[],c=roles.hasControls()?virtualclass.poll.dataToStd:virtualclass.poll.dataRec;for(const d in virtualclass.poll.count)b.push([c.options[d],virtualclass.poll.count[d]]),virtualclass.poll.count[d]&&(virtualclass.poll.showChart(),a&&(a.style.display="none"));for(const a in c.options)Object.prototype.hasOwnProperty.call(virtualclass.poll.count,
a)||(b.push([c.options[a],"0"]),virtualclass.poll.count[a]=0);virtualclass.poll.chart&&virtualclass.poll.chart.load({columns:b});roles.hasControls()&&this.updateVotingInformation()},updatePiChart(){const a=document.getElementById("pollResultMsz");this.updateVotingInformation();const b=roles.hasControls()?virtualclass.poll.dataToStd:virtualclass.poll.dataRec,c=[];for(const d in virtualclass.poll.count)c.push([b.options[d],virtualclass.poll.count[d]]),virtualclass.poll.count[d]&&(virtualclass.poll.showChart(),
a&&(a.style.display="none"));for(const a in b.options)Object.prototype.hasOwnProperty.call(virtualclass.poll.count,a)||(c.push([b.options[a],"0"]),virtualclass.poll.count[a]=0);virtualclass.poll.piChart&&virtualclass.poll.piChart.load({columns:c})},updateListResult(){this.updateVotingInformation();const a=virtualclass.poll.list.pop();virtualclass.poll.addResultListItem(a);virtualclass.poll.list.push(a);virtualclass.poll.pollState.data.list=virtualclass.poll.list},createPiChart(){const a=roles.hasControls()?
...
b){}}});virtualclass.poll.currResultView="pi";"undefined"!==typeof virtualclass.poll.pollState.data&&(virtualclass.poll.pollState.data.view="pi")},listView(){virtualclass.poll.currResultView="list";virtualclass.poll.hideChart();const a=document.getElementById("resultLayoutBody");var b=document.getElementById("listCont");b.hasChildNodes()&&b.removeChild(b.childNodes[0]);0<virtualclass.poll.list.length&&(virtualclass.poll.createResponseTable(a),(b=document.getElementById("pollResultMsz"))&&b.parentNode.removeChild(b));
...
addResultListItem(a){let b;const c=virtualclass.poll.dataToStd.options;var g=document.getElementById("resultList");for(var k in a)"username"!==k&&(b=a[k]);k=document.createElement("tr");g.appendChild(k);g=document.createElement("td");a=virtualclass.poll.capitalizeFirstLetterFnameLname(a.username);g.innerHTML=a;k.appendChild(g);g=document.createElement("td");g.innerHTML=c[b];k.appendChild(g)},capitalizeFirstLetterFnameLname(a){if(a&&null!=a){var [b,c]=a.split(/\s*(?: |$)\s*/);a=virtualclass.vutil.capitalizeFirstLetter(b);
...
(a.innerHTML=virtualclass.poll.dataToStd.question),roles.hasControls()&&(this.chartMenu(),this.createResultMsgCont())},createResultMsgCont(a){document.getElementById("pollResultMsz").innerHTML=virtualclass.lang.getString("watstdrespo")},pollClosedUI(){var a=document.getElementById("closePoll");a&&a.parentNode.removeChild(a);if(a=document.getElementById("congreaPollVoters"))a.innerHTML=virtualclass.lang.getString("rvtu");(a=document.getElementById("pollResultMsz"))&&a.parentNode.removeChild(a)},chartMenu(a){document.querySelector("#chartMenuCont #bar").addEventListener("click",
virtualclass.poll.barGraph);document.querySelector("#chartMenuCont #pi").addEventListener("click",virtualclass.poll.createPiChart);roles.hasControls()&&document.querySelector("#chartMenuCont #rList").addEventListener("click",virtualclass.poll.listView)},qnLabel(a){const b=document.createElement("div");b.id="chart";b.className="row";a.appendChild(b);console.log("====> chat graph, container ",document.querySelector("#chart").offsetWidth)},createNav(a){roles.hasControls()||document.querySelector(".congrea.student #virtualclassPoll #navigator a").addEventListener("click",
()=>{virtualclass.poll.showStudentPollReport()})},createMszBox(a){const b=document.createElement("div");b.id="mszBoxPoll";b.className="row";a.appendChild(b)},createOption(a,b){let c;const d=document.getElementById("optsTxCont").querySelectorAll("#optsTxCont .opt");let k=0;for(let a=0;a<d.length;a++){k++;var m=d[a].id}c=m?k:"0";m={};m.index=c;m.closeBtn=1<c;m=virtualclass.getTemplate("optioninput","poll")({close:m});document.querySelector("#addMoreCont").insertAdjacentHTML("beforebegin",m);document.getElementById(`remove${c}`).addEventListener("click",
()=>{virtualclass.poll.removeOption(b,a,`remove${c}`)})},hidePrevious(a){(a=document.getElementById("optsTxCont"))&&a.parentNode.removeChild(a);if(a=document.getElementById("reset"))a.style.display="none";a=document.getElementById("footerCtrCont");a.parentNode.removeChild(a)},editPoll(a,b){virtualclass.poll.UI.loadContent(a,b);virtualclass.poll.UI.footerBtns(a,b)},loadContent(a,c){let d;document.getElementById("qnTxCont");virtualclass.poll.UI.editQn(a,c);d=b.options;let g=0;if("object"===typeof d)for(var k in d)g++,
virtualclass.poll.UI.editOptions(a,c,k,g);else for(k=0;k<d.length;k++)virtualclass.poll.UI.editOptions(a,c,k,g)},previewFooterBtns(a,b,c){document.getElementById("footerCtrCont")&&virtualclass.poll.pollPopUp(virtualclass.poll.popupFn,c,b)},footerBtns(a,b){a&&virtualclass.poll.pollPopUp(virtualclass.poll.popupFn,b,a)},editQn(a,b){const c=document.querySelector("#qnTxCont #q");null==c&&(c.value=document.getElementById(`qnText${a}${b}`).innerHTML);null==c||c.value||(c.value="course"===a?virtualclass.poll.coursePoll[b].questiontext:
virtualclass.poll.sitePoll[b].questiontext)},editOptions(a,b,c,g){document.getElementById("optsTxCont").style.display="block";document.getElementById(`opt${b}${c}`);if(2<g){const d=document.createElement("a");d.id=`remove${c}`;d.className="close";d.innerHTML="&times";document.querySelector(`#optsTxCont .inputWrapper #option${c}`).parentNode.appendChild(d);d.addEventListener("click",()=>{virtualclass.poll.removeOption(a,b,d.id)})}g=document.getElementById(`option${c}`);"course"===a?(c=virtualclass.poll.coursePoll[b].options[c],
g.value="object"===typeof c?c.options:c):(c=virtualclass.poll.sitePoll[b].options[c],g.value="object"===typeof c?c.options:c)},pollSettingUI(a,b){const c=virtualclass.getTemplate("setting-modal","poll"),d=document.querySelector("#editPollModal");for(;d.firstChild;)d.removeChild(d.firstChild);d.insertAdjacentHTML("beforeend",c({time:function(a,b){const c=[];for(a=b-a+1;a--;)c[a]=b--;return c}(1,60)}));this.settingUIBody(a,b)},settingUIBody(a,b){this.addSettingHandlers()},addSettingHandlers(){document.getElementById("mode").addEventListener("click",
()=>{const a=document.getElementById("radioBtn1"),b=document.getElementById("radioBtn2");a.checked?(document.getElementById("timer").setAttribute("disabled",!0),document.getElementById("ut").setAttribute("disabled",!0)):b.checked&&(document.getElementById("timer").removeAttribute("disabled"),document.getElementById("ut").removeAttribute("disabled"))})},defaultLayoutForStudent(){this.container();const a=document.getElementById("mszBoxPoll");if(null==document.getElementById("stdPollMszLayout")){const b=
document.createElement("div");b.id="stdPollMszLayout";b.innerHTML=virtualclass.lang.getString("pollmaybeshown");a.appendChild(b)}},stdPublishUI(){const a=document.getElementById("stdPollMszLayout");a&&a.parentNode.removeChild(a)},disableClose(){document.getElementById("pollClose").style.display="none"}},updateUsersOnPoll(a){a.hasOwnProperty("users")?(virtualclass.poll.uniqueUsers=a.message.map(a=>a.userid),virtualclass.poll.uniqueUsers=virtualclass.poll.uniqueUsers.filter(a=>{if(a!==virtualclass.gObj.uid)return a}),
0<Object.keys(virtualclass.poll.count).length&&virtualclass.poll.updateVotingInformation()):0>virtualclass.poll.uniqueUsers.indexOf(virtualclass.jId)&&virtualclass.vutil.whoIsTeacher()!==virtualclass.jId&&(virtualclass.poll.uniqueUsers.push(virtualclass.jId),0<Object.keys(virtualclass.poll.count).length&&virtualclass.poll.updateVotingInformation())},makeGraphResponsive(){if(virtualclass.poll.pollState&&"stdPublishResult"===virtualclass.poll.pollState.currScreen&&virtualclass.poll.currResultView){let a;
(a="bar"===virtualclass.poll.currResultView?document.querySelector("#chartMenuCont #bar"):document.querySelector("#chartMenuCont #pi"))&&a.click()}}}};c.poll=b})(window);const dashboard={userConfirmation(c,b){virtualclass.popup.confirmInput(c,a=>{b(a)})},init(c,b){"SharePresentation"!==c||document.querySelector("#pptDbCont")||(null==document.querySelector(".docsDbCont")&&(document.querySelector("#SharePresentationDashboard").innerHTML=virtualclass.vutil.getPptDashBoard("SharePresentation")),virtualclass.sharePt.attachEvent("submitpurl","click",virtualclass.sharePt.initNewPpt),virtualclass.sharePt.ppts&&virtualclass.sharePt.ppts.length&&(virtualclass.sharePt.showPpts(virtualclass.sharePt.ppts),
...
identifyFirstAndLastNote(a){const b=virtualclass.dts.isFirstNote(a);a=virtualclass.dts.isLastNote(a);const c=document.querySelector("#screen-docs .pageContainer");b&&a?(c.classList.add("firstNote"),c.classList.add("lastNote")):b?(c.classList.remove("lastNote"),c.classList.add("firstNote")):a?(c.classList.remove("firstNote"),c.classList.add("lastNote")):(c.classList.remove("firstNote"),c.classList.remove("lastNote"))}}}})(window);(function(c){"use strtic";let b,a;c.quiz=function(){return{coursequiz:[],uid:virtualclass.gObj.uid,qzid:0,newUserTime:{},list:[],listQuiz:{},uniqueUsers:[],usersFinishedQz:[],quizJSON:{},attemptedUsers:{},cmid:0,quizSt:{},quizAttempted:{},initToFetch:!0,init(){virtualclass.previrtualclass="virtualclassQuiz";virtualclass.previous="virtualclassQuiz";this.cmid=virtualclass.vutil.getUrlVars(c.webapi).cmid;this.token=wbUser.token;!roles.hasAdmin()||roles.isEducator()?roles.isStudent()?this.UI.defaultLayoutForStudent():
...
c(k,m)})},publishHandler(a,b){const c=document.getElementById("mszBoxQuiz");0<c.childNodes.length&&c.childNodes[0].parentNode.removeChild(c.childNodes[0]);virtualclass.quiz.readyToPublishQuiz(a,b)},readyToPublishQuiz(a,b){this.openQuizPopup(a,b);this.quizPreview(a);this.displayQuizModalBox()},openQuizPopup(a,b){this.qzid=b;(a=document.getElementById("editQuizModal"))&&a.remove();a=document.getElementById("bootstrapQzCont");virtualclass.quiz.UI.generateModal("editQuizModal",a)},showQn(a){roles.hasControls()?
a.innerHTML="quiz question":a.innerHTML="quiz question for student"},action(a,b,c){b(a,c)},getQuizData(a){const d=this.coursequiz[a.id],g=new FormData;g.append("cmid",this.cmid);g.append("user",this.uid);g.append("qid",a.id);g.append("token",this.token);b=this;virtualclass.xhr.vxhr.post(`${c.webapi}&methodname=mod_congrea_get_quizdata`,g,{transformResponse:a=>a}).then(a=>{if(b.isJson(a.data))b.quizJSON=a.data,$("#slickQuiz").slickQuiz({json:b.quizJSON,questionPerPage:d.questionsperpage,questionMode:d.preferredbehaviour,
quizTime:d.timelimit,displayDetailResult:!1});else{const b=document.getElementById("publishQzBt");b.parentNode.removeChild(b);document.getElementById("contQzBody").innerHTML=a}}).catch(a=>{console.error("Request failed with error ",a)})},quizPopUp(a,b){const c=function(){virtualclass.quiz.action(this.id,a,b)},d=(document.getElementById("editQuizModal")?document.getElementById("editQuizModal"):document.getElementById("qzPopup")).querySelectorAll(":scope .controls");for(let a=0;a<d.length;a++)d[a].addEventListener("click",
...
a.setAttribute("aria-valuemax","100");a.style=`width:${b}%`;a.innerHTML=`${b}% Correct`;c.appendChild(a);return c},gradeReportLayout(){const a=virtualclass.view.customCreateElement("div","gradeRpBody","modal-body"),b=virtualclass.view.customCreateElement("table","","table");var c=virtualclass.view.customCreateElement("thead","","thead-inverse");const k=document.createElement("tr");var m=document.createElement("th");m.innerHTML="#";k.appendChild(m);m=document.createElement("th");m.innerHTML=virtualclass.lang.getString("gRname");
